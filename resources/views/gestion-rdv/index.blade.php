                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="fw-semibold">{{ $rdv->patient->user->name ?? 'N/A' }}</td>
                                <td>Dr. {{ $rdv->medecin->user->name ?? 'N/A' }}</td>
                                <td>{{ \Carbon\Carbon::parse($rdv->date_heure)->format('d/m/Y H:i') }}</td>
                                <td>
                                    <select onchange="updateStatut({{ $rdv->id }}, this.value)" class="form-select form-select-sm w-auto d-inline-block" style="width: auto;">
                                        <option value="en_attente" {{ $rdv->statut == 'en_attente' ? 'selected' : '' }} class="text-warning">🟡 En attente</option>
                                        <option value="confirme" {{ $rdv->statut == 'confirme' ? 'selected' : '' }} class="text-success">🟢 Confirmé</option>
                                        <option value="annule" {{ $rdv->statut == 'annule' ? 'selected' : '' }} class="text-danger">🔴 Annulé</option>
                                        <option value="termine" {{ $rdv->statut == 'termine' ? 'selected' : '' }} class="text-info">🔵 Terminé</option>
                                    </select>
                                </td>
                                <td>
                                    <form method="POST" action="{{ route('gestion-rdv.destroy', $rdv->id) }}" style="display:inline-block" 
